...
describe("Bridge", () => {
  ...
  it("swap: should be able to initiate the swap of 10 YAC tokens", async () => {
    ...
    expect(rSwap.events[1].args[1]).to.equal(10);
    expect(rSwap.events[1].args[2]).to.equal(0)
    expect(rSwap.events[1].args[3]).to.equal(false);
  ...
  it("redeem: should successfully redeem 10 YAC with nonce given the correct signature", async () => {
    let messageHash = ethers.utils.solidityKeccak256(
      ...
      [owner.address, bridge.address, 10, 0]
    );

    let signature = await owner.signMessage(ethers.utils.arrayify(messageHash));

    const txRedeem = bridge.connect(owner).redeem(signature, 10, 0);
    const rRedeem = await (await txRedeem).wait();

    expect(rRedeem.events[1].args[0]).to.equal(owner.address);
    expect(rRedeem.events[1].args[1]).to.equal(10);
    expect(rRedeem.events[1].args[2]).to.equal(0);
    expect(rRedeem.events[1].args[3]).to.equal(true);
  ...
  it("redeem: should revert given previously used nonce", async () => {
    ...
    await expect(bridge.connect(owner).redeem(signature, 10, 0)).to.be.revertedWith("ERROR: Nonce was used previously.");
  });

  it("redeem: should revert given an invalid signature", async () => {
    let messageHash = ethers.utils.solidityKeccak256(
      ["address", "address", "uint256", "uint256"],
      [owner.address, bridge.address, 10, 3]
    );

    let signature = await owner.signMessage(ethers.utils.arrayify(messageHash));

    await expect(bridge.connect(owner).redeem(signature, 10, 2)).to.be.revertedWith("ERROR: Signature is invalid.");
  });
